...
// Ruta: Editar una asignatura existente
app.put("/asignaturas/:id", async (req, res) => {
    ...
    try {
        ...
        await pool
            ...
            .input("creditos", sql.Int, creditos)
            .query(
                "UPDATE Asignaturas SET nombre = @nombre, creditos = @creditos WHERE id = @id"
            );
        res.send("Asignatura actualizada correctamente");
    } catch (err) {
        console.error("Error al actualizar asignatura:", err.message);
        res.status(500).send("Error al actualizar asignatura");
    }
});
...
app.delete("/asignaturas/:id", async (req, res) => {
    ...
    try {
        ...
        res.send("Asignatura eliminada correctamente");
    } catch (err) {
